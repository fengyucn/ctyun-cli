...
@click.option('--cycle-type', type=click.Choice(['MONTH', 'YEAR']), default=None,
              help='订购周期类型，包周期时必填')
@click.option('--cycle-count', type=int, default=None, help='订购周期大小(MONTH:1-60, YEAR:1-5)，包周期时必填')
...
@click.option('--sys-disk-size', type=int, default=None, help='系统盘大小(GB 40-2048)，VM时必填')
@click.option('--bandwidth', type=int, default=None, help='带宽(Mbps 1-2000)，IP时必填，VM/BMS可选')
...
@click.option('--disk-size', type=int, default=None, help='磁盘大小(GB 5-2000)，EBS时必填')
...
@click.option('--ip-pool-bandwidth', type=int, default=None,
              help='共享带宽大小(Mbps 5-2000)，IP_POOL时必填')
...
@click.option('--cbr-value', type=int, default=None,
              help='存储库大小(GB 100-1024000)，CBR_VM/CBR_VBS时必填')
@click.option('--output', type=click.Choice(['table', 'json', 'yaml']), default=None, help='输出格式')
@click.pass_context
def query_price(ctx, region_id: str, resource_type: str, count: int, on_demand: bool,
                cycle_type: Optional[str], cycle_count: Optional[int],
                flavor_name: Optional[str], image_uuid: Optional[str],
                sys_disk_type: Optional[str], sys_disk_size: Optional[int],
                bandwidth: Optional[int], disks: Optional[str],
                disk_type: Optional[str], disk_size: Optional[int], disk_mode: Optional[str],
                nat_type: Optional[str], ip_pool_bandwidth: Optional[int],
                device_type: Optional[str], az_name: Optional[str], order_disks: Optional[str],
                elb_type: Optional[str], cbr_value: Optional[int], output: Optional[str]):
    ...
    try:
        ...
        if disks:
            try:
                ...
                click.echo(f"错误: --disks JSON 格式无效: {e}", err=True)
                return
        ...
        if order_disks:
            try:
                ...
                click.echo(f"错误: --order-disks JSON 格式无效: {e}", err=True)
                return

        # 包周期参数校验
        if not on_demand:
            if not cycle_type:
                click.echo("错误: 包周期模式下 --cycle-type 为必填项 (MONTH/YEAR)", err=True)
                return
        ...
        if output_format == 'json':
            ...
            click.echo("=" * 60)
            click.echo(f"订单询价  资源类型: {resource_type}  数量: {count}")
            click.echo("=" * 60)
