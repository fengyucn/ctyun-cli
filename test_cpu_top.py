...
def test_cpu_top():
    ...
    if result.get('success'):
        ...
        if cpu_list:
            cpu_values = [float(item.get('value', 0)) * 100 for item in cpu_list]
            print(f"\n统计信息:")
            print(f"  最高CPU: {max(cpu_values):.2f}%")
            print(f"  最低CPU: {min(cpu_values):.2f}%")
            print(f"  平均CPU: {sum(cpu_values)/len(cpu_values):.2f}%")
    else:
        print(f"  error: {result.get('error')}")
        print(f"  message: {result.get('message')}")
